 ...
 */
BaseView::BaseView(Game *game, State *state, BaseViewClickHandler leftClickHandler, BaseViewClickHandler rightClickHandler, bool showExtraSpace, int width, int height, int x, int y) : InteractiveSurface(width, height, x, y), _game(game), _state(state), _leftClickHandler(leftClickHandler), _rightClickHandler(rightClickHandler), _showExtraSpace(showExtraSpace), _base(0), _texture(0), _selFacility(0), _big(0), _small(0), _lang(0), _gridX(0), _gridY(0), _selSize(0), _selector(0), _blink(true), _cameraPosX(0), _cameraPosY(0), isMouseScrolling(false), isMouseScrolled(false)
{
...
void BaseView::mousePress(Action *action, State *state)
{
	...
	if (((Uint8)-1) != Options::geoDragScrollButton)
	{
...
void BaseView::mouseClick (Action *action, State *state)
{
	...
	if (isMouseScrolling)
	{
		...
		{ // so we missed again the mouse-release :(
			...
			if ((!mouseMovedOverThreshold) && (SDL_GetTicks() - mouseScrollingStartTime <= ((Uint32)Options::dragScrollTimeTolerance)))
			{
	...
	if (isMouseScrolling)
	{
		...
		if (action->getDetails()->button.button == Options::geoDragScrollButton) isMouseScrolling = false; else return;
		// Check if we have to revoke the scrolling, because it was too short in time, so it was a click
		if ((!mouseMovedOverThreshold) && (SDL_GetTicks() - mouseScrollingStartTime <= ((Uint32)Options::dragScrollTimeTolerance)))
		{
			isMouseScrolled = false;
...
void BaseView::mouseOver(Action *action, State *state)
{
	if (isMouseScrolling && action->getDetails()->type == SDL_MOUSEMOTION)
	{
		...
		if (!mouseMovedOverThreshold)
			mouseMovedOverThreshold = ((std::abs(totalMouseMoveX) > Options::dragScrollPixelTolerance) || (std::abs(totalMouseMoveY) > Options::dragScrollPixelTolerance));
...
void BaseView::tryDismantle()
{
	if (_selFacility != 0)
	{
		...
		{
			_game->pushState(new ErrorMessageState(_state->tr("STR_FACILITY_IN_USE"), _state->getPalette(), Palette::blockOffset(15)+1, "BACK13.SCR", 6));
		}
		// Would base become disconnected?
		else if (!_base->getDisconnectedFacilities(_selFacility).empty())
		{
			_game->pushState(new ErrorMessageState(_state->tr("STR_CANNOT_DISMANTLE_FACILITY"), _state->getPalette(), Palette::blockOffset(15)+1, "BACK13.SCR", 6));
		}
		else
		{
			_game->pushState(new DismantleFacilityState(_base, this, _selFacility));
		}
	}
}
