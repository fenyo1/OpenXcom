 ...
 */
BuildFacilitiesState::BuildFacilitiesState(Base *base, State *state, int viewCameraPosX, int viewCameraPosY) : _base(base), _state(state), _facilities()
{
	_screen = false;
	...
	_view = new BaseView(_game, this, (BaseViewClickHandler)&BuildFacilitiesState::viewLeftClick, 0, Options::infiniteBaseSizes, 192, 192, 0, 8);
...
{
	_game->pushState(new PlaceFacilityState(_base, _facilities[_lstFacilities->getSelectedRow()], this, _view->getCameraPosX(), _view->getCameraPosY()));
}
